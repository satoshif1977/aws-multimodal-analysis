 ...
 *
 * 同ディレクトリの retry.ts / logger.ts と同じ「AWS 呼び出しの運用品質を揃える」
 * 方針の第 3 弾。retryMetrics() を withRetry の onRetry に渡して結線できる
 * （logger.ts の retryLogger() と同じシグネチャ）。
 * Bedrock / S3 / DynamoDB を 1 リクエストで続けて呼ぶため、どの段階で時間を
 * 使っているかをレイテンシのメトリクスで切り分けられるようにする。
