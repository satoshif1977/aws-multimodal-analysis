...
Bedrock の ThrottlingException や S3 / DynamoDB の一時的なサーバエラーに対して、
AWS 公式推奨の「指数バックオフ + フルジッター」方式で自動リトライする。

設計方針:
  - sleep / rand を注入可能にして、テストを決定的かつ高速に保つ
  - リトライ不能な例外（ValidationException 等）は即座に再送出する
  - 最終試行でも失敗した場合は元の例外をそのまま再送出する
    （呼び出し側の `except ClientError` を壊さないため、独自例外でラップしない）

使い方:
    result = retry_call(bedrock.invoke_model, modelId=..., body=...)
